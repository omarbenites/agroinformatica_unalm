...
# Leemos el archivo. skipemptyROws para saltar lineas en blanco
...
#dtpricePlot <- dplyr::slice(dct, 129:161) #2011, 2012, 2013
...
out <- left_join(x = dtsowa, y = dtharva, by = c("department","month")) %>% 
        left_join(.,dtproduction, c("department","month")) %>% 
          left_join(.,dtyield, c("department","month")) %>% 
            left_join(.,dtpricePlot, c("department","month"))
...
tbpapa <- left_join(dtsowa, dtharva, by = c("department", "month")) %>%
...
tbpapa <- tbpapa %>% mutate(year=2014)
...
# if (condicion) 
 #  sentecias de codigo
#}
# condicion: es un resultado que puede seer V o f
...
writeData(wb, sheet = cult_sheet, x = tbpapa)

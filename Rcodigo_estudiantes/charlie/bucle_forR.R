...
fp <- file.choose()
tbem_quinua <- readRDS(fp)

#agregamos la columna de nuestro cultivo (en ingles)

library(dplyr)
tbem_quinua <- mutate(tbem_quinua, crop = "quinoa")

#vemos la tabla en el visor
#vemos la estructura de la tabla

#Al ser pocas columnas, digitamos directos las columnas 2 al 6
for (i in 3:7){tbem_quinua[,i] <- as.numeric(tbem_quinua[,i])}

saveRDS(tbem_quinua, "tabla_quinua.rds")

# siguiente paso, subir el github

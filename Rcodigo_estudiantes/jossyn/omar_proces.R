...
# Funcion:juntar o agrupar la información 'gather(data, key, value)' (paquete tidyr): sirve para transponer las tablas 
# de forma ordenada.
...
tbarroz <- left_join(dtsowa, dtharva, by = c("department", "month")) %>%
            left_join(., dtproduction, by = c("department", "month")) %>% 
            left_join(., dtyield, by = c("department", "month")) %>%
            left_join(., dtpricePlot, by = c("department", "month"))
tbarroz <- tbarroz %>% mutate(year=2014)
